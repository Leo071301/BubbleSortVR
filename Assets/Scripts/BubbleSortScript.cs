...
public class BubbleSortScript : MonoBehaviour
{
    ...
    [SerializeField] public Color Good_Color;       // no swap needed
    [SerializeField] public Color Bad_Color;        // swap needed
    [SerializeField] public float Bad_TIME = 2.0f;  // swap highlight color time
        ...
        for (int i = 0; i < n - 1; i++)
        {
            for (int j = 0; j < n - i - 1; j++)
            {
                    ...
                    int.Parse(bubblesort_cubes[j + 1].name))
                {
                    ...
                    StartCoroutine(CubeUtility.PulseHighlight(bubblesort_cubes[j], Bad_Color, Bad_TIME));
                    StartCoroutine(CubeUtility.PulseHighlight(bubblesort_cubes[j + 1], Bad_Color, Bad_TIME));
    ...
    public void BubbleSortEvent()
    {
        ...
        bubblesort_cubes = CubeUtility.createCubeList(number_list); 
